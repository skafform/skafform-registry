...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  return { templates: getTemplates(process.cwd()) }
}
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  if (!title || !slug) return redirect("/admin/pages/new")

  await db.insert(skafformPages).values({ title, slug, template, content, status })
  return redirect("/admin/pages")
}

export default function AdminPagesNewPage({ loaderData }: { loaderData: { templates: Record<string, { label: string }> } }) {
      ...
      <form method="post">
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "var(--skafform-spacing-lg)" }}>
          ...
          <div>
            ...
            <input name="title" type="text" required placeholder="Ma page" style={inputStyle} />
          </div>

          <div>
            <label style={labelStyle}>Slug</label>
            <input name="slug" type="text" required placeholder="ma-page" style={inputStyle} />
            <p style={{ margin: "4px 0 0", fontSize: "var(--skafform-font-size-xs)", color: "var(--skafform-muted-fg)" }}>URL : /ma-page</p>
          </div>
